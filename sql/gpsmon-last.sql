...
-- Started on 2014-07-07 17:16:19 EEST
...
--
-- TOC entry 682 (class 3079 OID 12617)
...
--
-- TOC entry 683 (class 3079 OID 372729)
...
--
-- TOC entry 702 (class 1255 OID 332728)
...
CREATE FUNCTION gm_data_arh2fs() RETURNS SETOF timestamp with time zone
    ...
    AS $$
DECLARE
	begin_tstamp timestamp with time zone;
...
BEGIN
	select (date_trunc('month',CURRENT_TIMESTAMP) - (select "value" from settings where name='timeout_gpsarh2fs')::interval)::timestamp with time zone into begin_tstamp;
 -- изменить работу функции на использование целых месяцев для обработки
  -- функция сброса старых данных из gpsarh на файловую систему, ещё не проверялась в работе, так как небыло данных для сброса, необходимо протестировать
    ...
    FOR r IN select distinct date_trunc('day',tstamp) from data.gps_arh where tstamp < begin_tstamp
    LOOP
      file := date(r);
...
--
-- TOC entry 703 (class 1255 OID 339361)
...
DECLARE
	...
	insrow integer;
BEGIN
	select (date_trunc('month',CURRENT_TIMESTAMP) - (select "value" from settings where name='timeout_gps2arh')::interval)::timestamp with time zone into begin_tstamp;
	with instmp as (insert into data.gps_arh (select * from data.gps where tstamp < begin_tstamp) returning *) select count(*) into insrow from instmp;
	with deltmp as (delete from data.gps where tstamp < begin_tstamp returning *) select count(*) into delrow from deltmp;
...
--
-- TOC entry 759 (class 1255 OID 305941)
...
--
-- TOC entry 707 (class 1255 OID 998943)
...
from data.vtrackers_h_v016
where status=0
	and "class"=2
	and module_id=1
	and(tracker_imei,tracker_tstamp::integer) in (
select 
 tracker_imei as imei,
...
update data.online as t1 set 
 ...
 "location"='POINT('|| t2.lonr::varchar ||' '|| t2.latr::varchar||')'
from tmp1 as t2 
where t1.carid=t2.carid and t1.tstamp<=t2.tstamp and t2.latr>0;
...
update vtrackers_in set status=1 where id in (select distinct t1.id from data.vtrackers_h_v016 as t1,tmp1 as t2 where t1.tracker_imei=t2.imei and t1.class=2 and to_timestamp(t1.tracker_tstamp::integer)<=t2.tstamp);
...
--
-- TOC entry 708 (class 1255 OID 2266585)
-- Name: mod2_online_update_10(); Type: FUNCTION; Schema: data; Owner: gpsmon
--

CREATE FUNCTION mod2_online_update_10() RETURNS SETOF integer
    LANGUAGE plpgsql
    AS $$DECLARE
 res integer;
BEGIN
--  данном скрипте не учтено наличие 2-х трекеров у одного объекта мониторинга
SET TIME ZONE '0'; -- поправка временной зоны в UTC так как в ней трекер шлёт данные
-- создаем временную таблицу выбирая последние данные по машинам
drop table if exists tmp1;
create temporary table tmp1 on commit drop as 
select 
 ...
 (select trackerid from trackers where trackers.imei=tracker_imei) as trackerid,
 tracker_imei as imei,
 locations_location1_latitude::text as lat,
 locations_location1_longitude::text as lon,
 to_timestamp(tracker_tstamp,'YYYY-MM-DD HH24:MI:SS') as tstamp,
 "locations_location1_quality"::integer as gpsdop
from data.vtrackers_h_v016
where status=0
	and "class"=2
	and module_id=2
	and(tracker_imei,to_timestamp(tracker_tstamp,'YYYY-MM-DD HH24:MI:SS')) in (
select 
 tracker_imei as imei,
 max(to_timestamp(tracker_tstamp,'YYYY-MM-DD HH24:MI:SS')) as tstamp
 from data.vtrackers_h_v016
where
 status=0
 and "class"=2
 and module_id = 2
 and to_timestamp(tracker_tstamp,'YYYY-MM-DD HH24:MI:SS')<=now() + interval '1 day'
-- and to_number(locations_location1_latitude,'9')>0
 group by imei
);
SET TIME ZONE DEFAULT; -- возврат временной зоны, в нормальное состояние
...
-- gsmsignal=t2.gsmsignal,
 gpsdop=t2.gpsdop,
 tstamp_lastupdate=now(),
 who_lastupdate='mod2_online_update_10',
 "location"='POINT('|| t2.lon ||' '|| t2.lat ||')'
from tmp1 as t2 
where t1.carid=t2.carid and t1.tstamp<=t2.tstamp and t2.lat::real>0;

-- обновляем статус в json таблице для всех записей у которых время меньше чем online
-- тоесть помечаем исторические данные которые можно дальше обрабатывать
update vtrackers_in set status=1 where id in (select distinct t1.id from data.vtrackers_h_v016 as t1,tmp1 as t2 where t1.tracker_imei=t2.imei and t1.class=2 and to_timestamp(t1.tracker_tstamp,'YYYY-MM-DD HH24:MI:SS')<=t2.tstamp);
...
ALTER FUNCTION data.mod2_online_update_10() OWNER TO gpsmon;

--
-- TOC entry 709 (class 1255 OID 2275274)
-- Name: mod2_online_update_20(); Type: FUNCTION; Schema: data; Owner: gpsmon
--

CREATE FUNCTION mod2_online_update_20() RETURNS SETOF integer
    LANGUAGE plpgsql
    AS $$DECLARE
 res integer;
BEGIN
...
select 
 ...
 tracker_imei as imei,
 to_timestamp(tracker_tstamp,'YYYY-MM-DD HH24:MI:SS') as tstamp,
 sensors_power_internal_int1_percent::integer as battery_percent
from data.vtrackers_h_v016
where status=0
	and "class"=3
	and module_id=2
...
-- это дополнителоьная информация и вставлять новую строку согластно этим данным ненадо
-- --- --- вставляем в online таблицу новые записи по машинам которых там нет
--insert into data.online (carid) (select carid from tmp1 where carid not in (select carid from data.online));
...
-- gsmsignal=t2.gsmsignal,
-- gpsdop=t2.gpsdop,
 battery_percent=t2.battery_percent,
 tstamp_lastupdate=now(),
 who_lastupdate='mod2_online_update_20'
-- "location"='POINT('|| t2.lon ||' '|| t2.lat ||')'
from tmp1 as t2 
where t1.carid=t2.carid and t1.tstamp<=t2.tstamp;

-- обновляем статус в json таблице для всех записей у которых время меньше чем online
-- тоесть помечаем исторические данные которые можно дальше обрабатывать
update vtrackers_in set status=1 where id in (select distinct t1.id from data.vtrackers_h_v016 as t1,tmp1 as t2 where t1.tracker_imei=t2.imei and t1.class=3 and to_timestamp(t1.tracker_tstamp,'YYYY-MM-DD HH24:MI:SS')<=t2.tstamp);
...
ALTER FUNCTION data.mod2_online_update_20() OWNER TO gpsmon;

SET search_path = public, pg_catalog;

--
-- TOC entry 697 (class 1255 OID 17978)
...
--
-- TOC entry 699 (class 1255 OID 327867)
...
--
-- TOC entry 698 (class 1255 OID 17979)
...
--
-- TOC entry 700 (class 1255 OID 972936)
...
--
-- TOC entry 2303 (class 1255 OID 17980)
...
--
-- TOC entry 2304 (class 1255 OID 17981)
...
--
-- TOC entry 677 (class 1259 OID 1867286)
...
--
-- TOC entry 675 (class 1259 OID 1867193)
...
--
-- TOC entry 674 (class 1259 OID 1867191)
...
--
-- TOC entry 676 (class 1259 OID 1867284)
...
--
-- TOC entry 678 (class 1259 OID 1867685)
...
--
-- TOC entry 679 (class 1259 OID 1867717)
...
--
-- TOC entry 680 (class 1259 OID 2209398)
-- Name: vtrackers_h_v016; Type: VIEW; Schema: data; Owner: gpsmon
--

CREATE VIEW vtrackers_h_v016 AS
 SELECT trackers.id,
    trackers.tstampdb,
    trackers.status,
    json_extract_path_text(trackers.json_data, VARIADIC ARRAY['jsonversion'::text]) AS jsonversion,
    (json_extract_path_text(trackers.json_data, VARIADIC ARRAY['class'::text]))::integer AS class,
    (json_extract_path_text(trackers.json_data, VARIADIC ARRAY['module'::text, 'id'::text]))::integer AS module_id,
    json_extract_path_text(trackers.json_data, VARIADIC ARRAY['module'::text, 'name'::text]) AS module_name,
    json_extract_path_text(trackers.json_data, VARIADIC ARRAY['module'::text, 'info'::text]) AS module_info,
    json_extract_path_text(trackers.json_data, VARIADIC ARRAY['module'::text, 'version'::text]) AS module_version,
    to_timestamp(((json_extract_path_text(trackers.json_data, VARIADIC ARRAY['module'::text, 'tstamp'::text]))::integer)::double precision) AS module_tstamp,
    ...
    json_extract_path_text(trackers.json_data, VARIADIC ARRAY['locations'::text, 'location1'::text, 'quality'::text]) AS locations_location1_quality,
    json_extract_path_text(trackers.json_data, VARIADIC ARRAY['sensors'::text, 'temperature'::text, 'tracker'::text]) AS sensors_temperature_tracker,
    json_extract_path_text(trackers.json_data, VARIADIC ARRAY['sensors'::text, 'power'::text, 'external'::text, 'ext1'::text, 'volt'::text]) AS sensors_power_external_ext1_volt,
    json_extract_path_text(trackers.json_data, VARIADIC ARRAY['sensors'::text, 'power'::text, 'external'::text, 'ext1'::text, 'percent'::text]) AS sensors_power_external_ext1_percent,
    json_extract_path_text(trackers.json_data, VARIADIC ARRAY['sensors'::text, 'power'::text, 'internal'::text, 'int1'::text, 'volt'::text]) AS sensors_power_internal_int1_volt,
    json_extract_path_text(trackers.json_data, VARIADIC ARRAY['sensors'::text, 'power'::text, 'internal'::text, 'int1'::text, 'percent'::text]) AS sensors_power_internal_int1_percent,
    json_extract_path_text(trackers.json_data, VARIADIC ARRAY['sensors'::text, 'dut'::text, 'dut1'::text]) AS sensors_dut_dut1,
    json_extract_path_text(trackers.json_data, VARIADIC ARRAY['sensors'::text, 'dut'::text, 'dut2'::text]) AS sensors_dut_dut2
   FROM trackers;


ALTER TABLE data.vtrackers_h_v016 OWNER TO gpsmon;
...
--
-- TOC entry 681 (class 1259 OID 2267035)
-- Name: online; Type: VIEW; Schema: public; Owner: gpsmon
--
...
CREATE VIEW online AS
 SELECT online.carid,
    online.trackerid,
    online.tstamp,
    gis.st_y(online.location) AS lat,
    gis.st_x(online.location) AS lon,
    online.speed,
    online.dest_day,
    online.dut,
    online.speed_max,
    ...
    online.gsmsignal,
    online.gpsdop,
    online.tstamp_lastupdate,
    online.who_lastupdate,
    online.battery_percent AS batttery_percent
   FROM data.online
  WHERE ((gis.st_y(online.location) IS NOT NULL) AND (gis.st_x(online.location) IS NOT NULL));


ALTER TABLE public.online OWNER TO gpsmon;
...
--
-- TOC entry 4598 (class 2604 OID 1867289)
...
--
-- TOC entry 4597 (class 2604 OID 1867196)
...
--
-- TOC entry 4596 (class 2604 OID 1705657)
...
--
-- TOC entry 4587 (class 2604 OID 354687)
...
--
-- TOC entry 4591 (class 2604 OID 972462)
...
--
-- TOC entry 4576 (class 2604 OID 18105)
...
--
-- TOC entry 4577 (class 2604 OID 18106)
...
--
-- TOC entry 4578 (class 2604 OID 18107)
...
--
-- TOC entry 4580 (class 2604 OID 18108)
...
--
-- TOC entry 4581 (class 2604 OID 18109)
...
--
-- TOC entry 4585 (class 2604 OID 18110)
...
--
-- TOC entry 4633 (class 2606 OID 1867201)
...
--
-- TOC entry 4635 (class 2606 OID 1867294)
...
--
-- TOC entry 4623 (class 2606 OID 18153)
...
--
-- TOC entry 4631 (class 2606 OID 1705662)
...
--
-- TOC entry 4603 (class 2606 OID 1705664)
...
--
-- TOC entry 4627 (class 2606 OID 354693)
...
--
-- TOC entry 4629 (class 2606 OID 972467)
...
--
-- TOC entry 4605 (class 2606 OID 1705811)
...
--
-- TOC entry 4607 (class 2606 OID 18116)
...
--
-- TOC entry 4609 (class 2606 OID 18118)
...
--
-- TOC entry 4611 (class 2606 OID 18120)
...
--
-- TOC entry 4613 (class 2606 OID 18122)
...
--
-- TOC entry 4615 (class 2606 OID 18124)
...
--
-- TOC entry 4617 (class 2606 OID 18126)
...
--
-- TOC entry 4619 (class 2606 OID 18128)
...
--
-- TOC entry 4624 (class 1259 OID 1705666)
...
--
-- TOC entry 4625 (class 1259 OID 1705694)
...
--
-- TOC entry 4620 (class 1259 OID 18154)
...
--
-- TOC entry 4621 (class 1259 OID 18155)
...
--
-- TOC entry 4599 (class 1259 OID 18129)
...
--
-- TOC entry 4600 (class 1259 OID 18130)
...
--
-- TOC entry 4601 (class 1259 OID 18131)
...
--
-- TOC entry 4636 (class 2606 OID 1867295)
...
-- Completed on 2014-07-07 17:16:30 EEST
