...
-- удаляем из таблицы приёма данных те записи imei которых не зарегестрирован в базе
delete from data.gps_from_trackers where imei not in (select imei from trackers);
select * into temporary gps_ft from data.gps_from_trackers where class=2;
...
-- удаляем из временной таблицы дублирующие записи
...
select distinct imei,max(tstamp) as tstamp into temporary tmp1_imei from tmp1 where carid is null group by imei;
...
select imei,tstamp into temporary tmpnotdb from tmp1 where carid is null;
...
--select imei,max(tstamp) as tstamp,null::integer as lon,null::integer as lat into temporary gpsf_tmp1 from data.gps_from_trackers group by imei;
--update gpsf_tmp1 set lon=,lat= from 
--select carid,max(tstamp),null::integer as lon,null::integer as lat into temporary gps_tmp1 from data.gps group by carid;
...
select * into temporary tmp22 from tmp2;
update tmp22 set dest=0,speed=0 where speed<5;
select carid,max(tstamp) as tstamp ,max(speed) as speed_max,sum(dest) as dest into temporary tmp3 from tmp22 where tstamp >= date_trunc('day', CURRENT_TIMESTAMP) group by carid;
