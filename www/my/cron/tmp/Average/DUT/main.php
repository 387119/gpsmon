<?php
	$db_conn=ConnectToPsql('db1','gps','tgps','tgpspwd');
...
#	$tstamp_begin="'1970-01-01 0:0:0'"; # если база чистая надо начать с самого начала, указываем явную дату
	$sql = "insert into olap.gps_average_day (carid,day,speed_max,dest,speed_sum,counts) (select carid,date_trunc('day',tstamp)::date as day,max(speed),sum(dest),sum(speed),count(*) from data.gps_arh where speed>5 and lat>0 and lon>0 and tstamp < $tstamp_end and tstamp >= $tstamp_begin group by day,carid) returning carid,day;";
	$res = pg_query($sql);
	if($res==false)die();
...
<?php
			...
			//for($y1=0;$y1<sizeof($days);$y1++){
				...
				for($i=0;$i<count($stops);$i++){
					...
					if($tank['tank1']==true){$sql1="select tstamp, dut[1]  as dut1
						from data.gps_arh where tstamp>='".$stops[$i]['tstampb']."'::timestamp with time zone + interval 'minute 1' and tstamp<='".$stops[$i]['tstamp']."'::timestamp with time zone  and carid=".$cars[$i1]['carid']." and lat>0 and lon>0 and dut[1]>".$tank['dut1_min']." and speed<5 order by tstamp;";
					}
					if($tank['tank2']==true){$sql2="select tstamp,  dut[2] as dut2
						from data.gps_arh where tstamp>='".$stops[$i]['tstampb']."'::timestamp with time zone + interval 'minute 1' and tstamp<='".$stops[$i]['tstamp']."'::timestamp with time zone  and carid=".$cars[$i1]['carid']." and lat>0 and lon>0 and dut[2]>".$tank['dut2_min']." and speed<5 order by tstamp;";
					}
					if($tank['tank1']==true){
	...
	function GetTimeMoveStop($conn,$table,$cars){
		for($i=0;$i<sizeof($cars);$i++){
			...
			$sql = "select tstamp,speed from data.gps_arh where carid=".$cars[$i]['carid']." and tstamp>='".$tstamp_start."' and tstamp<='".$tstamp_end."' and lat>0 and lon>0 order by tstamp;";
			$res = pg_query($sql);
			$out['move']=0;
			$out['stop']=0;
			$time_prev=false;
