...
<body style="font: 9pt Tahoma">
    <?
        ...
        foreach ($_REQUEST['cars'] as $car => $value) {
            ...
            $query = "SELECT date_part('epoch', tstamp) epoch, gsmsignal, gpsdop FROM data.gps WHERE carid=$car AND tstamp>='$datefrom $timefrom' AND tstamp<='$dateto $timeto' ORDER BY epoch";
            ...
            foreach ($result_array as $track) {
                ...
                $gsm_data .= '['.$epoch.','.round($track[gsmsignal] * 100 / 32).'],';
                $gps_data .= '['.$epoch.','.($track[gpsdop] > 0 ? abs(round($track[gpsdop] * 100 / 50) - 100) : 0).'],';
            }

            if (empty($gps_data) && empty($gsm_data)) {
                ...
                echo "
                    ...
                    <script type='text/javascript'>
                        $(function () {
                            ...
                            var data = [
                                { data: gsm_data, label: \"Уровень сигнала GSM, %\", color: \"#33CCFF\"},
                                { data: gps_data, label: \"Уровень сигнала GPS, %\", color: \"#660099\"}
                            ]; 
                                ...
                                if (item) {
                                    if (previousPoint != item.dataIndex) {
                                        ...
                                        switch (item.seriesIndex) {
                                            case 0: // gsm
                                                showTooltip(item.pageX, item.pageY, d.getHours() + \":\" + d.getMinutes() + \" - GSM: \" + y + \"%\");
                                                break;
                                            case 1: // gps
                                                showTooltip(item.pageX, item.pageY, d.getHours() + \":\" + d.getMinutes() + \" - GPS: \" + y + \"%\");
                                                break;
                                        }
                                    }
                                }
