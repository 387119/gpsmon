	...
	foreach ($_REQUEST['cars'] as $car => $value) {
		...
		foreach ($arr_stops as $i=>$stop) {
			...
			//$reverse_geo = file_get_contents('http://maps.googleapis.com/maps/api/geocode/json?latlng=48.33417333333333,38.40648333333333&sensor=false&language=ru');
                    $stops['latitude'] = $stop['lat'];
                    $stops['longitude'] = $stop['lon'];
                    //$str = "http://maps.googleapis.com/maps/api/geocode/json?latlng=".$lat.",".$lon."&sensor=false&language=ru";
                    //echo $str;
                    //$reverse_geo = file_get_contents('$str');
			//$reverse_geo = file_get_contents('http://maps.googleapis.com/maps/api/geocode/json?latlng='.($lat).','.($lon).'&sensor=false&language=ru');
			//$reverse_geo = file_get_contents('http://maps.googleapis.com/maps/api/geocode/json?latlng='.($stops['latitude']).','.($stops['longitude']).'&sensor=false&language=ru');
                	//$reverse_geo = file_get_contents('http://geocode-maps.yandex.ru/1.x/?geocode=48.123123,38.1231231');
                    echo $reverse_geo;
